...
public class GalacticWar extends Applet implements Runnable, KeyListener {
	...
	public void init() {
		...
		astExplosion = new AnimatedSprite(this, g2d);
		astExplosion.load("explosion96x96x16.png", 4, 4, 96, 96);
		astExplosion.setFrameDelay(2);
		astExplosion.setAlive(false);
	...
	public void drawExplosions() {
		...
		if (astExplosion.alive()) {
			astExplosion.updateAnimation();
			if (astExplosion.currentFrame() == astExplosion.totalFrames() - 1) {
				astExplosion.setCurrentFrame(0);
				astExplosion.setAlive(false);
			} else {
				astExplosion.draw();
			}
	...
	private void drawCollisionLocations() {
		for (int i = 0; i < collisionLocations.size(); i++) {
			...
			g2d.setColor(Color.WHITE);
			g2d.drawString("Collosion at: " + Math.round(collisionLocation.X())
					+ "," + Math.round(collisionLocation.Y()),
					SCREENWIDTH - 400, i * 15 + 10);
		}
	...
	public void startExplosion(Sprite sprite) {
		...
		public void startAstExplosion(Sprite sprite) {
		if (!astExplosion.alive()) {
			double x = sprite.position().X() ;//- sprite.getBounds().width / 2;
			double y = sprite.position().Y() ;//- sprite.getBounds().height / 2;
			astExplosion.setPosition(new Point2D(x, y));
			astExplosion.setCurrentFrame(0);
			astExplosion.setAlive(true);
		}
	...
	public void handleAsteroidCollisions() {
		for (int n = 0; n < ASTEROIDS; n++) {
			if (ast[n].state() == STATE_COLLIDED) {
				ast[n].setState(STATE_EXPLODING);
				startAstExplosion(ast[n]);
			}
		}
